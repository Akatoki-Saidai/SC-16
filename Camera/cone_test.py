...
4.何epochのが一番いいかまだよく見てないので、それを使うようにする。
...
final_boxes = None
# ONNXセッション
session = onnxruntime.InferenceSession("C:\\Users\\konan\\Downloads\\yolox_nano_cone_best.onnx")
while(cap.isOpened()):
    # フレームを取得
    ret, frame = cap.read()
    origin_img = frame
    ...
    if dets is not None:#コーンが映った場合、BoundingBoxが描画される
        ...
        print(float(x_left_upper[0]) + float(x_left_upper[2]) / 2)
